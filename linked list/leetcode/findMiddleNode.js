...
class LinkedList {
  ...
  findMiddleNode() {
    // initialize two pointers, we're gonna use tortoise and hare algorithms
    let pre = this.head
    let temp = this.head

    while(temp !== null && temp.next !== null) {
      pre = pre.next // move slow pointer one step
      temp = temp.next.next // move fast pointer two steps
    }

    return pre // return middle node
  }
}
